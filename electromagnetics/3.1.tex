\section{电磁场问题的求解} \label{3}
本章考虑稳恒态电磁场问题, 也即电磁场分布与时间 $t$ 无关. 此时电磁场问题可以归纳为三类:
\begin{itemize}
    \item 静电场问题. 基本方程为
          \begin{gather*}
              \frac{\partial\rho}{\partial t}=0, \\
              \bm{J}=0, \\
              \nabla\times\bm{E}=0.
          \end{gather*}
    \item 导体恒电流问题, 也即直流电路. 基本方程为
          \begin{gather*}
              \nabla\cdot\bm{J}=-\frac{\partial\rho}{\partial t}=0, \\
              \nabla\times\bm{E}=0.
          \end{gather*}
    \item 静磁场问题, 也即由 $\bm{J}$ 产生的磁场.
\end{itemize}

\subsection{静电场}
\subsubsection{静电场的基本方程和边界条件}
静电场问题的基本方程为
\begin{equation} \label{eq:3.1 1}
    \begin{cases}
        \nabla\cdot\bm{D}=\rho, \\
        \nabla\times\bm{E}=0.
    \end{cases} \qquad
    \begin{cases}
        \oint_S\bm{D}\cdot\mathrm{d}\bm{S}=Q, \\
        \oint_C\bm{E}\cdot\mathrm{d}\bm{l}=0.
    \end{cases}
\end{equation}
其中存在本构关系 $\bm{D}=\epsilon\bm{E}$.

在边界曲面的上下各取一面积 $\Delta S$ 的面, 法向量分别取向上或向下. 二者可形成一个封闭体, 并假设其高度 $d$ 无限趋近 0. 根据麦克斯韦方程组的积分形式 (\ref{eq:2.6 m int}), 可以得到
\begin{equation} \label{eq:3.1 basic 1}
    \begin{aligned}
                    & \oint_\Sigma\bm{B}\cdot\mathrm{d}S=0                             \\
        \Rightarrow & \bm{B_1}\cdot\bm{e_n}\Delta S+\bm{B_2}\cdot(-\bm{e_n})\Delta S=0 \\
        \Rightarrow & B_{1n}=B_{2n}.
    \end{aligned}
\end{equation}

同理,
\begin{equation} \label{eq:3.1 basic 2}
    \begin{aligned}
                    & \oint_\Sigma\mathrm{D}\cdot\mathrm{d}\bm{S}=Q \\
        \Rightarrow & \bm{e_n}\cdot(\bm{D_1}-\bm{D_2})=\rho_S.
    \end{aligned}
\end{equation}
其中, 对于一般物质, $\rho_S=0$; 对于导体, 电介质或 $\sigma_1=\sigma_2$ 时, $\rho_S\neq0$. 此时, 也即
\begin{equation}
    \epsilon_1E_{1n}=\epsilon_2E_{2n}.
\end{equation}

类似地, 在边界一处取一垂直的 (以看向纸面内部的角度而言) 逆时针闭合回路. 取垂直边界向上为 $\bm{e_n}$, 纸面角度沿边界向右为 $\bm{e_t}$, 指向纸面内的基矢 $\bm{e_p}$. 闭合环路中垂直边界高度 $h$ 且不用考虑, 水平边界长度设为 $l$, 则得到
\begin{equation*}
    \begin{aligned}
                    & \oint_C\bm{E}\cdot\mathrm{d}\bm{l}=-\frac{\mathrm{d}}{\mathrm{d}t}\int\bm{B}\cdot\mathrm{d}\bm{S}=0 \\
        \Rightarrow & -\bm{E_1}\cdot l\bm{e_t}+\bm{E_2}\cdot l\bm{e_t}=0                                                  \\
        \Rightarrow & (\bm{E_1}-\bm{E_2})\cdot\bm{e_t}=0.
    \end{aligned}
\end{equation*}
也即
\begin{equation} \label{eq:3.1 basic 3}
    \bm{e_n}\times(\bm{E_1}-\bm{E_2})=0.
\end{equation}

对于磁场强度, 有
\begin{gather}
    \begin{aligned}
          & \oint_C\bm{H}\cdot\mathrm{d}\bm{l}                  \\
        = & (-\bm{H_1}+\bm{H_2})\cdot l\bm{e_t}                 \\
        = & \bm{e_p}\cdot(\bm{e_n}\times(-\bm{H_1}+\bm{H_2}))l,
    \end{aligned} \\
    \begin{aligned} \label{eq:3.1 6}
          & \int_\Sigma\left(\bm{J}+\frac{\partial\bm{D}}{\partial t}\right)\cdot\mathrm{d}\bm{S}                                                          \\
        = & \bm{J}lh\cdot(-\bm{e_p})+\frac{\mathrm{d}}{\mathrm{d}t}\left(\bm{D_1}\cdot\frac{lh}{2}(-\bm{e_p})+\bm{D_2}\cdot\frac{lh}{2}(-\bm{e_p})\right).
    \end{aligned}
\end{gather}

对于式 (\ref{eq:3.1 6}), 对 $t$ 求导为零. 考虑\textit{理想导体模型}, $\bm{J_h}=\bm{J_S}$. 所以有
\begin{equation} \label{eq:3.1 basic 4}
    \bm{e_n}\times(\bm{H_1}-\bm{H_2})=\bm{J_S}.
\end{equation}

由此, 得到静电场问题的边界条件 (无需考虑初始条件) 为式 (\ref{eq:3.1 basic 1}), (\ref{eq:3.1 basic 2}), (\ref{eq:3.1 basic 3}), (\ref{eq:3.1 basic 4}).

\subsubsection{静电位的边界条件} \label{3.1.2}
容易得到 (\ref{eq:3.1 1}) 可以化为
\begin{equation} \label{eq:3.1 2}
    \bm{E}=-\nabla\varphi(\bm{r}). \qquad \text{(因为 $\nabla\times(\nabla u)\equiv 0$.)}
\end{equation}

注意到 (\ref{eq:3.1 1}) 和本构关系式可推出
\begin{equation}
    \nabla\cdot(\epsilon\bm{E})=\epsilon(\bm{r})\nabla\cdot\bm{E}(\bm{r})+\bm{E}(\bm{r})\nabla\cdot\epsilon(\bm{r})=\rho.
\end{equation}
假设在各向同性均匀线性介质中, 则有
\begin{equation} \label{eq:3.1 3}
    \epsilon\nabla\cdot\bm{E}=\rho.
\end{equation}

将 (\ref{eq:3.1 2}) 代入 (\ref{eq:3.1 3}), 得到
\begin{equation} \label{eq:3.1 4}
    \nabla^2\varphi(\bm{r})=-\frac{\rho}{\epsilon}.
\end{equation}
上式 (\ref{eq:3.1 4}) 是一个关于电位 $\varphi(\bm{r})$ 的泊松方程, 其中 $\rho$ 包含自由电荷 $\rho_f$ 和极化电荷 $\rho_P$, 且都为位矢 $\bm{r}$ 的标量函数. 给定边界条件之后, 即可得到唯一解. 若空间中无电荷, 则
\begin{equation}
    \nabla^2\varphi(\bm{r})=0.
\end{equation}
这是一个拉普拉斯方程.

继续考虑式 (\ref{eq:3.1 4}), 可以得到
\begin{equation}
    \varphi(\bm{r})=-\frac{1}{4\pi\epsilon}\int\frac{\rho(\bm{r'})}{|\bm{r}-\bm{r'}|}\mathrm{d}r'.
\end{equation}

电位的参考点应当满足: 无穷远处或接地.

\textbf{介质分界面}

设 $P_1,P_2$ 是介质分界面两侧紧贴界面的相邻两点, 则
\begin{align*}
                & \varphi_1-\varphi_2  =\lim_{\Delta l\rightarrow 0}\int_{P_1}^{P_2}\bm{E}\mathrm{d}\bm{l}=0 \\
    \Rightarrow & \varphi_1            =\varphi_2.
\end{align*}

同时,
\begin{equation} \label{eq:3.1 5}
    \bm{e_n}(-\epsilon_1\nabla\varphi_1+\epsilon_2\nabla\varphi_2)=\epsilon_2\frac{\partial\varphi_2}{\partial n}-\epsilon_1\frac{\partial\varphi_1}{\partial n}=\rho_S.
\end{equation}
若分界面上无自由电荷 ($\rho_S=0$), 则
\begin{equation}
    \epsilon_2\frac{\partial\varphi_2}{\partial n}=\epsilon_1\frac{\partial\varphi_1}{\partial n}.
\end{equation}

\textbf{导体分界面}

设导体的介电常数为 $\epsilon_2$. 因为导体内部场强为零, 所以
\begin{equation}
    \nabla\varphi=0.
\end{equation}

由式 (\ref{eq:3.1 5}),
\begin{equation}
    \epsilon_1\frac{\partial\varphi_1}{\partial n}=-\rho_S.
\end{equation}

\textbf{静电场问题的分析思路}
\begin{itemize}
    \item 分析电荷边界的对称性;
    \item 已知应对一般问题的基本方程, 确定边界条件.
\end{itemize}

\textbf{静电能}

可以证明
\begin{equation}
    W=\frac{1}{2}\int_V\varphi(\bm{r})\rho(\bm{r})\mathrm{d}V.
\end{equation}

\begin{exampleprob}
    真空中, 两块无限大接地导体平板分别置于 $x=0$ 和 $x=a$ 处, 在两板之间的 $x=b$ 处有一面密度 $\rho_{S0}$ 的均匀电荷分布的板. 求两导体平板之间的电位和电场.

    \begin{solution}
        设 $0<x<b$ 和 $b<x<a$ 之间的电位分别为 $\varphi_1(x),\varphi_2(x)$. 除 $x=b$ 外其余空间无电荷分布, 故电位函数满足一维拉普拉斯方程:
        \begin{equation*}
            \begin{cases}
                \frac{\mathrm{d}^2\varphi_1(x)}{\mathrm{d}x^2}=0, & \quad 0<x<b; \\
                \frac{\mathrm{d}^2\varphi_2(x)}{\mathrm{d}x^2}=0, & \quad b<x<a.
            \end{cases}
        \end{equation*}

        由数理方程知识, 方程组的解为
        \begin{equation*}
            \begin{cases}
                \varphi_1(x)=C_1x+D_1, \\
                \varphi_2(x)=C_2x+D_2.
            \end{cases}
        \end{equation*}

        考虑边界条件,
        \begin{equation*}
            \begin{cases}
                \varphi_1(0)=0,                                                                                                                                                                        \\
                \varphi_2(a)=0,                                                                                                                                                                        \\
                \varphi_1(b)=\varphi_2(b),                                                                                                                           & \quad \text{(介质分界面)}            \\
                \left.\left(\frac{\mathrm{d}\varphi_2(x)}{\mathrm{d}x}-\frac{\mathrm{d}\varphi_1(x)}{\mathrm{d}x}\right)\right|_{x=b}=-\frac{\rho_{S0}}{\epsilon_0}. & \quad \text{(式 \ref{eq:3.1 5})}
            \end{cases}
        \end{equation*}

        综上, 解得
        \begin{equation*}
            \begin{cases}
                C_1=-\frac{\rho_{S0}(b-a)}{\epsilon_0a}, \\
                D_1=0,                                   \\
                C_2=-\frac{\rho_{S0}b}{\epsilon_0a},     \\
                D_2=\frac{\rho_{S0}b}{\epsilon_0}.
            \end{cases}
        \end{equation*}

        因此,
        \begin{equation*}
            \begin{cases}
                \varphi_1(x)=\frac{\rho_{S0}(a-b)}{\epsilon_0a}x, & \quad 0<x<b; \\
                \varphi_2(x)=\frac{\rho_{S0}b}{\epsilon_0a}(a-x), & \quad b<x<a.
            \end{cases}
        \end{equation*}
        并且
        \begin{equation*}
            \begin{cases}
                \bm{E_1}(x)=-\frac{\rho_{S0}(a-b)}{\epsilon_0a}\bm{e_x}, & \quad 0<x<b; \\
                \bm{E_2}(x)=\frac{\rho_{S0}b}{\epsilon_0a}\bm{e_x},      & \quad b<x<a.
            \end{cases}
        \end{equation*}
    \end{solution}
\end{exampleprob}
